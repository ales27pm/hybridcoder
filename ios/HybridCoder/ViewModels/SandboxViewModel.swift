...
final class SandboxViewModel {
        ...
        do {
            ...
            if let loadedLegacy: [SandboxProject] = try await secureStore.getObject(legacyStorageKey, as: [SandboxProject].self) {
                ...
                do {
                    ...
                    logger.error("Failed to persist migrated Keychain legacy projects: \(error.localizedDescription)")
                }
                return
            }
    ...
    func openProject(_ project: StudioProject) {
        ...
        let openedProjectID = project.id
        Task {
            await saveProjectsWithLogging()
            let loadedState = await stateMemory.loadState(for: openedProjectID)
            guard self.activeStudioProject?.id == openedProjectID else { return }
            if let loadedState, loadedState.projectID == openedProjectID {
    ...
    func closeProject() {
        if let projectToPersist = activeStudioProject {
            let stateSnapshot = restoredState
            Task {
                await persistCurrentState(for: projectToPersist, using: stateSnapshot)
            }
        }
    ...
    func duplicateProject(_ project: StudioProject) async {
        let duplicateFiles = project.files.map {
            StudioProjectFile(path: $0.path, content: $0.content, language: $0.language)
        }
    ...
    func saveActiveEditorState(fileID: UUID?, cursorPosition: Int?, tab: String?) async {
        ...
        var state = stateForActiveProject(project)
    ...
    func exportStateFromProjectFolder(_ projectID: UUID, sourceRoot: URL) async {
        if let state = await stateMemory.exportStateFromProjectFolder(projectID: projectID, sourceRoot: sourceRoot) {
            if activeStudioProject?.id == projectID, state.projectID == projectID {
                restoredState = state
            }
        }
    }
    ...
    private func persistCurrentState(
        ...
        using existingState: PrototypeStateMemory.ProjectState?
    ) async {
        let stateSeed: PrototypeStateMemory.ProjectState
        if let existingState, existingState.projectID == project.id {
            stateSeed = existingState
        } else {
            stateSeed = PrototypeStateMemory.ProjectState(
    ...
    private func saveProjectsWithLogging() async {
        do {
            try await saveProjects()
        } catch {
    ...
    private func legacyTemplateID(for template: SandboxProject.TemplateType) -> String {
        switch template {
        case .blank:
            return "blank_expo_js"
        case .helloWorld:
            return "blank_expo_js"
        case .navigation:
            return "stack_starter"
        case .todoApp:
            return "todo_app"
        case .apiExample:
            return "api_example"
    ...
    private func stateForActiveProject(_ project: StudioProject) -> PrototypeStateMemory.ProjectState {
        if let restoredState, restoredState.projectID == project.id {
            return restoredState
        }

        return PrototypeStateMemory.ProjectState(
            projectID: project.id,
            conversationSnippets: [],
            lastSavedAt: Date()
        )
