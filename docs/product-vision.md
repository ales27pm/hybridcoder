...
| Bytecoding runtime | Partial milestone plus bounded runtime loop: planner/coordinator plus validate -> replan -> retry exist, retries stay goal-aware and skip already-completed non-patch write actions, execution is still patch-centric for many writes, and KPI instrumentation is persisted locally with exported validation reports rather than full scenario-suite attainment. | Full agent-runtime loop with intent decomposition, first-class actions, iterative validation, and retry. |
| Workspace actions | Goal-derived create/overwrite/append/prepend/insert-before/insert-after/replace-between/replace-text/delete-text/rename/delete plus create/rename/delete-folder and move-file paths are real, missing-file direct/append/prepend updates now bootstrap into guarded create writes, delete/rename/move retries treat already-applied target state as idempotent success, deterministic patch-plan writes now attempt direct execution before PatchEngine fallback, and patch-backed writes refresh cached file snapshots so follow-on same-plan edits observe current content; patch-backed modify remains integrated for richer update scenarios. | First-class create/modify/rename/delete/move actions available consistently in runtime execution. |
...
| --- | --- | --- | --- |
| Time to first scaffold output | Persisted local runtime KPI snapshot (p50, partial sample coverage) | <= 120s | For a valid chat scaffold request, first coherent multi-file Expo output is produced within target time. |
| Goal-to-plan latency (p50) | Persisted local runtime KPI snapshot (p50) | <= 15s | Runtime emits an ordered action plan within target latency for Phase 6 scenario inputs. |
| Multi-step task completion without manual file edits | Persisted local runtime KPI snapshot (sampled multi-step scenarios) | >= 70% (Phase 6 scenario set) | Scenario finishes with runtime-executed workspace actions and no required manual file intervention. |
| Preview truthfulness | Persisted local preview truthfulness audit counter (validation-time checks) | 0 false runtime claims in validation suite | Diagnostic/preview surfaces never claim full RN runtime capability where not implemented. |
| Workspace safety | Persisted local runtime KPI counter (escaped-path violations) | 0 out-of-bound file actions | Validation suite records no file create/modify/rename/delete/move escaping workspace boundaries. |
